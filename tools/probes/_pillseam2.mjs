...
await p.evaluate(() => { document.documentElement.classList.add('drawer-open'); const st=document.createElement('style'); st.textContent='.btn-pill-delete::before{background:rgba(255,0,0,.85)!important;border-color:#0f0!important}'; document.head.appendChild(st); });
...
const box = await p.evaluate(() => {
  ...
  return { x: b1.right - 30, y: b1.top - 4, width: (b2.right - b1.right) + 38, height: b1.height + 8 };
});
await p.screenshot({ path: 'D:/VSCode projects/DUSK_v2.0/audit-v2/shots/_pillseam2.png', clip: box });
