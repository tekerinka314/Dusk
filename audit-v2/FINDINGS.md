...
### V2-B11-04 — воркер: `/exchange` и `/refresh` обслуживают ЛЮБОЙ origin (CORS прячет ответ только от браузера), лимитов нет; `/ws` пускает кого угодно
- **Evidence:** A (code, `worker/src/index.js:32-44,95-112`). **НЕ ИСПРАВЛЯЛОСЬ** —
  `worker/` вне пуш-блока по правилам исполнения STRATEGY.
...
### V2-B11-06 — 🔻 ОТКРЫТО (нужен вердикт юзера): у `updatedAt` из чужого файла/кривых часов нет потолка → запись с датой из будущего выигрывает КАЖДЫЙ мерж навсегда
...
- **Почему не исправлено:** клампинг = изменение семантики мержа (`09-sync`),
  которую правила запрещают трогать без ратифицированной спеки. **Предложение к
  вердикту:** клампить на ГРАНИЦЕ (не в мерже) входящий `updatedAt` до
  `min(v, Date.now() + 24ч)` и писать об этом в лог панели синка.

---

### V2-B12-01 — 🔻 ОТКРЫТО (нужен вердикт по арту): иконки объявлены `any maskable`, но арт нарушает 80%-ю безопасную зону → на Android маска срежет углы тома
...
- **Развилка юзеру (вкус, не решаю сам):** (а) отдельный maskable-вариант — тот же
  том, уменьшенный до ~62% на полнокровном фоне, нынешний остаётся `purpose:"any"`;
  (б) оставить как есть и принять срез углов; (в) переработать силуэт под маску.
