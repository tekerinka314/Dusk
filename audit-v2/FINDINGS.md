...
## Batch B11 — Security · Batch B12 — PWA (экспресс-аудит новых поверхностей, 2026-07-28, Opus 5)

Метод: adversarial-линзы инлайн (без субагентов — вердикт юзера «как качественнее»);
статические свипы + РАНТАЙМ-зонд на happy-dom с реальным подъёмом модулей 01-08
(рецепт `tests/quickadd-cyrillic.test.mjs`). Правило сессии соблюдено: **сперва
замер, потом код** — до фикса каждая инъекция подтверждена настоящим `<img>` в DOM.
Одноразовые свипы (скретчпад): `b11_htmlsinks.mjs` (116 HTML-стоков),
`b11_userdata.mjs` (290 HTML-шаблонов → 21 интерполяция пользовательских полей),
`b11_attrs.mjs` (интерполяции в контексте атрибута).

---

### V2-B11-01 — ⚠ ФЛАГМАН БАТЧА: `id` записи из недоверенного источника едет в атрибут СЫРЫМ → хранимый XSS (ПОДТВЕРЖДЁН ЗОНДОМ, ИСПРАВЛЕН)
- **Evidence:** B (runtime). Зонд (временный `tests/_b11probe.test.mjs`, снят после
  фикса) сеял `id = 'x"><img src=q onerror="window.__pwned=1">'` и звал НАСТОЯЩИЕ
  рендеры: лист Гримуара `renderGrimList()` → **1 `<img>` в DOM**; меню образцов
  `_grimRenderTplMenu()` → **2**; строка обета `createTaskEl()` → **10** (id
  подставляется в десяток атрибутов строки). После фикса — 0 везде.
- **Severity:** UI 2 · DL 2 · RR 1 · IC 1 · CF 3
- **Where:** интерполяции `data-id/data-nid/data-gid/data-tid="${…}"` без `escHtml`
  (~90 точек: `02-grimoire.ts:1076,1163-1200,1426,4085-4088`, `03-render.ts`,
  `04-tasks.ts`). Сами точки — СЛЕДСТВИЕ; корень ниже.
- **Root cause (это и чинилось):** в приложении ТРИ границы доверия, ведущие в один
  и тот же `state`: (A) импорт свитка `importData` (`03-render.ts:1471`),
  (B) посадка мержа синка `applySyncSubset` (`11-sync-ui.ts:318`, файл Drive),
  (C) чтение LS/IDB при буте. Форму проверяла ТОЛЬКО (A) — и то текст/витраж/
  перечисления (`sanitizeTask`/`sanitizeGroup`), а `id`, `deadline` и всё прочее
  пролезало через `{...t}`. Общий пост-шлюз `normalizeState()` (01-core), который
  зовётся после КАЖДОЙ подмены state, идентичность только БЭКФИЛЛИЛ (`uid`,
  `updatedAt`), но НЕ ПРОВЕРЯЛ. Рендеры при этом резонно исходили из «id всегда
  int/uuid» — значит правильное место лечения одно, и это шлюз.
- **Failure scenario:** юзер читает свиток от другого человека (или его файл Drive
  подменён) → запись с кривым `id` рисуется на любой из четырёх поверхностей →
  инъекция исполняется в origin приложения. **Усиление:** `refresh_token` лежит в
  `localStorage` (`10-cloud.ts:91`, залоченный размен) → инъекция читает его и
  через НАШ же публичный `/refresh` воркера получает бессрочный доступ к Drive
  appDataFolder. Класс не «поехала вёрстка», а «утёк ключ от синка».
- **FIX (2026-07-28):** `_sanitizeIdentity()` в `01-core.ts`, вызывается ПЕРВОЙ
  строкой `normalizeState()` → одна проверка закрывает все три границы. int-`id`
  обетов/звеньев/сводов/образцов, не-int → свежий из счётчика; uuid-`id` записей и
  образцов Гримуара сверяется с `^[A-Za-z0-9_.:-]{1,64}$` (в него укладывается ВСЁ,
  что минтит `uid()` — и `crypto.randomUUID()`, и фоллбэк `n-…`), иначе минтится
  заново; витраж всюду обязан быть `#RRGGBB` (у свода → дефолт `#6C8EF5`, у
  обета/записи → `null`); `groupId` не-int → `null`.
  Замок: `tests/untrusted-shape.test.mjs` (7 кейсов, включая «честную форму НЕ
  трогать» — иначе шлюз сам стал бы источником потери данных).

### V2-B11-02 — тела «Летописи» минуют санитайзер NA-2: чужой слепок исполняется в предпросмотре и садится в саму запись (ПОДТВЕРЖДЁН, ИСПРАВЛЕН)
- **Evidence:** B (runtime). Зонд: `_grimVersions` из «чужого» файла →
  `_grimRenderHistory()` → `<img onerror>` в DOM летописи.
- **Severity:** UI 2 · DL 1 · RR 1 · IC 1 · CF 3
- **Where:** `02-grimoire.ts` — `_grimRestoreVersions()` копировал `src[id].slice()`
  дословно; `_grimRenderHistory()` вставлял `${sel.b}` сырым; `grimHistRestore()`
  писал `note.body = v.b` без чистки.
- **Root cause:** решение NA-2 «один доверенный шлюз на границе данных» вживлено в
  `migrateNotes()`, а тот ходит ТОЛЬКО по `state.notes`/`state.notesArchive`.
  Летопись живёт в ОТДЕЛЬНОМ ключе LS (`K_NOTE_VERSIONS`, вне `state`) — и потому
  оказалась вне шлюза, хотя приезжает тем же импортом (`_grimVersions` в свитке).
  Урок класса: «шлюз на границе» перечислять ПО ХРАНИЛИЩАМ, а не по одному объекту.
- **FIX (2026-07-28):** `_grimSanitize` в трёх точках — на границе импорта
  (`_grimRestoreVersions`), в предпросмотре летописи и при откате к слепку
  (последние две = вторая линия, ловят и правленый вручную LS). Замок — те же
  `tests/untrusted-shape.test.mjs` (кейсы 6-7).

### V2-B11-03 — витраж свода интерполируется в `style="…"` сырым; проверялся только на импорте, на посадке синка — нет (ИСПРАВЛЕН тем же шлюзом)
- **Evidence:** A (code) + B (шлюз-тест). Severity UI 1 · DL 0 · RR 1 · IC 1 · CF 3
- **Where:** `03-render.ts:1904,1926-1928` (`style="background:${bg};color:${g.color}…"`,
  `data-bg="${bg}"`) и `02-grimoire.ts:1042` (`--nc:${n.color}`). `hexToRgb()`
  деградирует в дефолт, но САМ `g.color` подставляется рядом без проверки.
- **Failure scenario:** свод из подменённого файла Drive с `color: 'red" onmouseover="…'`
  разрывает атрибут. Импорт такое отсекал (`sanitizeGroup`), синк — нет.
...
- **Суть:** `corsHeaders` при чужом Origin отдаёт `allowed[0]` — БРАУЗЕР ответ
  ...
  `refresh_token` меняет его на access-токен через наш эндпойнт, т.е. воркер
  работает оракулом `client_secret`. Ни rate-limit, ни потолка размера тела.
  `/ws` (WebSocket вне CORS вовсе): комната = SHA-256 от fileId, то есть НЕ
  угадывается (`12-sync-wake.ts:50` — проверено, это сделано верно), но
  подключение неаутентифицировано, число комнат не ограничено, сообщение
  ретранслируется пирам ДОСЛОВНО без проверки размера/типа.
- **Рекомендация (в пуш-блок, где воркер и так открывают):** серверный отказ 403 на
  чужой Origin (а не только заголовок), простой rate-limit по IP/KV, потолок
  размера тела и сообщения `/ws`, свежий `compatibility_date` (сейчас 2025-06-01)
  + `observability` в `wrangler.toml`.

### V2-B11-05 — диспетчер `data-act` резолвил действия ПО ПРОТОТИПУ (`constructor`, `toString`) (ИСПРАВЛЕН, hardening)
- **Evidence:** A (code, `01-core.ts:1602`). Severity UI 0 · DL 0 · RR 1 · IC 0 · CF 3
- **Суть:** `map[el.dataset.act]` на обычном объекте: `data-act="toString"` доставал
  функцию из `Object.prototype` и звал её. Сегодня безвредно (такие функции ничего
  не делают с `(el,e)`), но это лишняя дверь рядом с B11-01.
- **FIX:** `hasOwnProperty` + проверка `typeof fn === 'function'`.

### V2-B11-06 — ✅ ИСПРАВЛЕНО 2026-07-28 (`ca705eb`) — у `updatedAt` из чужого файла/кривых часов не было потолка → запись с датой из будущего выигрывала КАЖДЫЙ мерж навсегда
- **Evidence:** A (code) + D (вывод из семантики мержа). Severity UI 1 · DL 2 · RR 2 · IC 1 · CF 2
- **Суть:** локальные часы монотонны и НЕ отравляемы (`_lastTs` стартует с 0,
  `01-core.ts:1502`) — проверено. Но входящий `updatedAt` не клампится ничем:
  запись со штампом `9e15` (подложенный свиток ИЛИ реальное устройство с часами,
  сбитыми в будущее) при поле-в-поле мерже «новее выигрывает» бьёт любую будущую
  локальную правку, а проигравшая правка уходит в карантин — молча и бесконечно.
- **FIX (по вердикту юзера 2026-07-28):** кламп поставлен на ГРАНИЦЕ, в том же
  шлюзе `_sanitizeIdentity` — всё, что дальше `Date.now() + 24ч`, срезается: у
  обетов/звеньев/сводов/образцов/записей (`updatedAt`/`createdAt`/`archivedAt`), у
  надгробий (`deletedAt`) и у журнала карантина (`at`/`resolvedAt`) — по их штампам
  работает GC, дата из будущего пережила бы любой TTL. Семантика `09-sync` НЕ
  тронута. Потолок считается от `Date.now()`, а не от `nowTs()`: монотонные часы
  сами подтянулись бы за отравленным штампом. Число срезанных пишется в лог панели
  синка и в `globalThis._clampedStamps`.
  ⚠ **Оговорка (осознанная):** тот мерж, в котором отравленный штамп приехал
  ВПЕРВЫЕ, он ещё выигрывает — кламп срабатывает уже на посадке. Начиная со
  следующей локальной правки победа переходит к живому устройству.
  Замки: `tests/untrusted-shape.test.mjs` (+2 кейса, включая «честное недавнее
  будущее в пределах суток НЕ трогаем»).

---

### V2-B12-01 — ✅ ИСПРАВЛЕНО 2026-07-28 (`23b65b5`): иконки объявляли `any maskable`, но арт нарушал 80%-ю безопасную зону → под маской Android том терял плечи и цоколь
- **Evidence:** A (геометрия арта) — `public/icon-192.svg`/`icon-512.svg`,
  `manifest.json` `"purpose": "any maskable"`.
- **Severity:** UI 2 (на Android) · DL 0 · RR 1 · IC 2 · CF 3
- **Расчёт:** безопасная зона maskable = круг радиусом 40% стороны (на 192 — r=76.8
  от центра 96,96). Углы тома: (24,54) → r=83.4; (24,168) → r=101.8; (168,168) →
  r=101.8. Все ВНЕ круга → лаунчер с круглой/скруглённой маской режет плечи и
  подножие тома. Сейчас это не видно (десктопный инсталл маску не применяет), но
  ровно этот арт уедет в **TWA APK (блок 5)** как иконка лаунчера и сплэш.
- **FIX (юзер отдал выбор на качество/надёжность 2026-07-28) — вариант (а),
  ОТДЕЛЬНЫЙ maskable-файл.** Почему он: нынешняя иконка остаётся нетронутой там, где
  маски нет (десктоп, браузер, вкладка), а Android получает арт, который не режется
  НИ ОДНОЙ маской; силуэт при этом не перерисован — та же группа, только масштаб,
  поэтому знак остаётся тем же символом (вариант «в» ломал бы узнаваемость ради
  одной платформы, вариант «б» отдавал бы качество). `public/icon-maskable.svg`,
  масштаб 0.74 подобран по КРАЙНИМ точкам арта (дальняя точка 188.7 при пороге
  204.8), проверено рендером под круглой и squircle-маской. Прекеш SW пополнен,
  CACHE `dusk-shell-v9` → `v10`.
  Отвергнутые варианты: (б) оставить как есть — принятая потеря качества на главной
  целевой платформе блока 5; (в) переработать силуэт под маску — расхождение знака
  с остальным приложением.
- **Проверено и ОПРОВЕРГНУТО по ходу:** гипотеза «Bubblewrap/TWA не умеет SVG-иконки»
  ЛОЖНА — `packages/core/src/lib/ImageHelper.ts` явно ловит `image/svg` и гонит
  через `svg2img` перед Jimp. Формат менять не требуется.

### V2-B12-02 — в манифесте не было `id` (личность приложения висела на `start_url`) (ИСПРАВЛЕНО)
- **Evidence:** A. Severity UI 1 · DL 0 · RR 1 · IC 0 · CF 3
- **Суть:** без `id` Chrome выводит идентификатор установленного приложения из
  `start_url`. Любая будущая правка `start_url` (её уже правили в B1-19:
  `./index.html` → `./`) делает установленную копию ДРУГИМ приложением. Добавлено
  `"id": "/"` — резолвится в тот же `https://dusk-du4.pages.dev/`, который Chrome
  выводит сейчас, поэтому установленные копии не ломаются, а якорь появляется ДО
  выпуска APK.

### V2-B12-03 — прекеш-контракт СХОДИТСЯ (позитив, зафиксирован как инвариант)
- **Evidence:** B. `npm run build` → `dist/` = `_headers, app.js, bg-gothic.jpg,
  icon-192.svg, icon-512.svg, index.html, manifest.json, pen-asset.js, style.css,
  sw.js, version.json`. `CORE_ASSETS` (8) + `OPTIONAL_ASSETS` (2) покрывают всё,
  кроме `_headers`/`sw.js`/самого себя → холодный офлайн-бут полон. Цепочка
  свежести цела: `_headers` → `/version.json: no-store`, `/sw.js: no-cache`; SW
  отдаёт `version.json` только из сети с офлайн-фоллбэком; поллер (index.html) сам
  себя лечит при неудачном первом чтении и не долбит скрытую вкладку. Внешних
  зависимостей у офлайна две — шрифт JetBrains Mono и `accounts.google.com/gsi/client`;
  обе async и деградируют.

### V2-B12-04 — переносимый файл всё ещё несёт SW-регистрацию и 5-секундный поллер `version.json` (низкий)
- **Evidence:** B (`grep` по `dist/dusk-portable.html`: `serviceWorker` ×3,
  `version.json` ×1). Severity UI 0 · DL 0 · RR 1 · IC 1 · CF 3
- **Суть:** на `file://` `navigator.serviceWorker` отсутствует (гард срабатывает), а
  `fetch('./version.json')` вечно падает в `catch` — тост не показывается, но
  консоль засоряется каждые 5 с. Косметика; чинится вырезанием обоих блоков в
  `scripts/build-portable.mjs`. Отложено как отдельная единица работы.
