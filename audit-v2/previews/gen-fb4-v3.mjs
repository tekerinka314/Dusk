...
const PS15 = {
  ...
  // а сломан ровно тот элемент, которым ранг и задаётся, — ШПИЛЬ: он срезан по
  // диагонали, от него остался клин, накренённый влево, справа на земле лежит
  // рухнувший блок. Читается как «та же башня, только обезглавленная».
  // Правка №3: клин-обрубок читался как целая деталь, а не как СЛОМ. Добавлено то,
  // чего не хватало вердикту, — сам излом и обломки: верх шпиля обрывается ЗУБЦАМИ
  // разной высоты через выщербину (5→6, провал на 6→7, уцелевший зуб 7→5), а
  // справа у подножия лежит отвалившийся клин шпиля, плюс скол слева. Корпус,
  // перемычка и цоколь по-прежнему совпадают с тремя рангами символ в символ.
  none: `<svg viewBox="0 0 15 15" fill="currentColor">`
    + `<path d="M5 8V6H6V7H7V5H8V8Z"/>`
    + `<rect x="5" y="8" width="5" height="1"/>`
    + `<rect x="5" y="9" width="2" height="3"/>`
    + `<rect x="8" y="9" width="2" height="3"/>`
    + `<path d="M10.5 12H13V9.5Z" opacity="0.9"/>`
