...
const PA = {
  ...
  // Руина, ред. 4 (2026-07-25): корпус цитирует ранги символ в символ — те же
  // стены на 8.4/15.6 и та же перемычка на y12; сломан ВЕРХ. Левая стена уцелела
  // выше и оборвана надломом, правая обрушена ниже, в проёме лежит рухнувший
  // клин кровли. Детализация даётся сколами, а не переизобретением силуэта.
  none: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">` +
    `<line x1="8.4" y1="18.6" x2="8.4" y2="8.6"/>` +
    `<line x1="15.6" y1="18.6" x2="15.6" y2="10.6" stroke-width="1.35" opacity="0.9"/>` +
    `<path d="M9.6 12H14.4" stroke-width="0.85" opacity="0.45"/>` +
    `<path d="M8.4 9.6L9.7 8.7" stroke-width="0.8" opacity="0.5"/>` +
    `<path d="M15.6 11.6L14.5 10.9" stroke-width="0.8" opacity="0.5"/>` +
    `<path d="M10.2 18.6L13.8 18.6L12 16Z" fill="currentColor" stroke="none" opacity="0.7"/>` +
    PLINTH + GROUND + `</svg>`,
};
...
const PS15 = {
  ...
  // Правка №2 по вердикту 2026-07-25: обе прежние руины не опознавались рядом с
  // тремя башнями, потому что ломали НЕ ТО. Узнавание держится на трёх общих
  // деталях — цоколь, перемычка-архитрав шириной 5 и двойные стены со щелью;
  // именно их прежние варианты и разрушали, оставляя абстрактные обломки.
  // Теперь корпус башни цел ЦЕЛИКОМ и совпадает с тремя рангами символ в символ,
  // а сломан ровно тот элемент, которым ранг и задаётся, — ВЕРХ.
  // Ред. 4 (2026-07-25, заказ «сложнее и читаемее»): ровный срез читался как
  // замысел, а не как повреждение. Теперь верх обрывается РАЗНОВЫСОКО — левая
  // стена уцелела на 3 клетки выше архитрава, правая обрушена почти вровень с
  // ним, — и у подножия справа лежат два обломка разной плотности, слева щебень.
  // Асимметрия и есть сигнал разрушения; общая часть силуэта не тронута.
  none: `<svg viewBox="0 0 15 15" fill="currentColor">`
    ...
    + `<rect x="11" y="10" width="1" height="1" opacity="0.6"/>`
    + `<rect x="11" y="11" width="1" height="1"/>`
    + `<rect x="3" y="11" width="1" height="1" opacity="0.55"/>`
    + PS_BASE15 + `</svg>`,
};
