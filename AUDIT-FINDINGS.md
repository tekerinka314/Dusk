...
- [ ] **P-B. Вид «Сегодня».** Кнопка-тогл в тулбаре → предикат в `filterAndSort` (overdue+сегодня по `deadlineStatus`) + сортировка schedule-режима. Без новой страницы. Риск: низкий.
- [ ] **P-C. Кольцевой бэкап + восстановление.** Обёртка над `saveState` (троттл N мин) → кольцо ~7–10 снимков в отдельном ключе LS + модалка восстановления. Мост к синку, делать ПЕРЕД Idea 8. Риск: низкий (следить за квотой).
- [ ] **P-D. Паритет массовых действий.** В main-select-bar добавить массовую смену группы/цвета/дедлайна (по образцу `bulkSetPriority` `app.js:1857`). Риск: низкий.
